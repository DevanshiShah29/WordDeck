...
export default function VocabCard({
  ...
  // Open the shared Sidebar via the miniwordmap:open event
  const openSidebar = (e) => {
    e?.preventDefault();
    e?.stopPropagation();
    try {
      window.dispatchEvent(
        new CustomEvent("miniwordmap:open", {
          detail: { slug, label: word, emitterId: "vocabcard" },
        }),
      );
    } catch (err) {
      console.error("Failed to dispatch sidebar open event", err);
    }
  };
        ...
        <div className="flex flex-col h-full group relative bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-500 overflow-hidden cursor-pointer transform hover:-translate-y-1 border border-slate-100">
          ...
          <div className="relative h-40 sm:h-48 md:h-50 ">
            ...
            <div className="absolute top-4 left-4 z-10">
              <div
                onClick={openSidebar}
                title={`More info about ${word}`}
                variant="transparent"
                className={`px-3 py-2 rounded-full text-xs font-bold shadow-md bg-gradient-to-r ${typeGradient} text-white capitalize`}
              >
                <span className="inline-flex items-center gap-2">
                  <span className="capitalize">{type}</span>
                  <Info className="w-4 h-4 inline-block text-white" />
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex-grow p-5 space-y-4">
            ...
            <div className="flex items-start justify-between mb-2">
              <h3 className="flex-1 min-w-0 text-xl font-bold text-[var(--slate-900)] py-1 group-hover:text-[var(--primary-600)] transition-colors duration-300 truncate">
                {word}
              </h3>
